...
def messegeGenerator(nameList, detailList, SML):
    messege = format(str(
        'Dear Parent, %0A Kindly Find CIE 2 Marks: %0A {unn} : {usn} %0A {nme} : {name} %0A {sn1} : {sub1}/{sm1} %0A {sn2} : {sub2}/{sm2} %0A {sn3} : {sub3}/{sm3} %0A {sn4} : {sub4}/{sm4} %0A {sn5} : {sub5}/{sm5} %0A {sn6} : {sub6}/{sm6} %0A {sn7} : {sub7}/{sm7} %0A Regards, %0A Hemavati, Mentor %0A 3rd Sem AIML'))
    ...
    messege = messege.format(
        sm1=SML[0], sm2=SML[1], sm3=SML[2], sm4=SML[3], sm5=SML[4], sm6=SML[5])
    return messege
