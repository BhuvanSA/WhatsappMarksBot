...
# This needs to be changed to work for 2nd IA
    ...
    searchbar = WebDriverWait(browser, 200).until(EC.presence_of_element_located((By.XPATH, search_xpath)))
...
# This needs to be changed to work for 2nd IA
            ...
            for col in range(5,sheet.max_column + 1,2):
            ...
            walink = "https://web.whatsapp.com/send?phone=+91"+str(studentRow[-1])+"&text="+messege+"&app_absent=1"
            ...
            try: 
                    ...
                    sheet.cell(row,sheet.max_column+1).font = Font(color='00ff0000')
...
def messegeGenerator(nameList,detailList,SML):
    messege = format(str('Dear Parent, %0A Kindly Find CIE 1 Marks: %0A {unn} : {usn} %0A {nme} : {name} %0A {sn1} : {sub1}/{sm1} %0A {sn2} : {sub2}/{sm2} %0A {sn3} : {sub3}/{sm3} %0A {sn4} : {sub4}/{sm4} %0A {sn5} : {sub5}/{sm5} %0A {sn6} : {sub6}/{sm6} %0A {sn7} : {sub7}/{sm7} %0A Regards, %0A Class Teacher %0A 3rd Sem AIML' )) 
    messege = messege.format(sm1=SML[0],sm2=SML[1],sm3=SML[2],sm4=SML[3],sm5=SML[4],sm6=SML[5],sm7=SML[6],unn=nameList[0],usn=detailList[0],nme=nameList[1],name=detailList[1],sub1=detailList[2],sub2=detailList[3],sub3=detailList[4],sub4=detailList[5],sub5=detailList[6],sub6=detailList[7],sub7=detailList[8],sn1=nameList[2], sn2=nameList[3], sn3=nameList[4], sn4=nameList[5], sn5=nameList[6], sn6=nameList[7], sn7=nameList[8])
    return messege
...
headlabel = Label(root,padx=5,text="Welcome to Whatsapp Marks Bot",font=("",30),fg="black",bg="lightgrey")
headlabel.grid(row=1)


btnLabel = Label(root,text="Select the Excel file to start",font=("",30),bg="lightgrey",fg="black")
btnLabel.grid(row=2)


button = Button(root, padx=5 ,text="Select", command=main,font = ("",24),bg="white",fg="black")
button.grid(row=3)
